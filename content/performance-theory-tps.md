...
| 테스트 유형 | Scouter에서 확인할 지표 | 분석 포인트 |
|-------------|-------------------------|-------------|
| **스파이크 테스트** | Active Service 그래프, XLog 응답 시간 | Active Service가 수직으로 솟구칠 때, 응답 시간(XLog)이 비정상적으로 길어지는 구간을 찾아 '임계 시간'을 설정하는 근거로 삼으세요. |
| **가용성 테스트** | CPU/Memory 사용률, Active Service | 서버 한 대가 다운되었을 때 나머지 서버들의 CPU/Memory가 급격히 상승하는지 관찰하여, 최소 필요 서버 대수를 산정하세요. |
| **로드 테스트** | XLog, Active Service, Heap Memory | TPS가 일정한데 응답 시간이 점점 길어진다면, 서버 내부에서 대기열(Queue)이 쌓이고 있다는 강력한 신호입니다. |
| **스트레스 테스트** | CPU 100%, GC 발생 빈도, Thread Pool | CPU가 100%에 도달하거나 GC가 빈번하게 발생하면, 해당 자원이 병목 지점일 수 있습니다. |